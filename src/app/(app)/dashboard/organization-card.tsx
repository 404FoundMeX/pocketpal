        ...
        <div className="flex justify-between">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start">
              ...
              {organizations.data?.map((org: { id: string; name: string }) => (
                <DropdownMenuItem
                  className="py-1"
                  key={org.id}
                  onClick={async () => {
        ...
        <div className="flex flex-col gap-8 md:flex-row">
          <div className="flex flex-grow flex-col gap-2">
            ...
            <div className="flex flex-col gap-2">
              {optimisticOrg?.members.map((member: { id: string; user: { image?: string; name?: string }; role: string }) => (
                <div
                  key={member.id}
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                                              <p className="text-sm">{member.user.name ?? "Unknown User"}</p>
                        <p className="text-muted-foreground text-xs">
                          {member.role}
                        </p>
                    </div>
              ...
              {!optimisticOrg?.id && (
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm">{session?.user.name ?? "Unknown User"}</p>
            ...
            <Button
              onClick={async () => {
                const invite = organization.inviteMember({
                  ...
                  fetchOptions: {
                    throw: true,
                    onSuccess: ({ data }: { data: { id: string; organizationId: string; email: string; role: "member" | "admin" | "owner"; status: "pending" | "accepted" | "rejected" | "canceled"; inviterId: string; expiresAt: Date } }) => {
                      if (optimisticOrg) {
                        setOptimisticOrg({
                          ...optimisticOrg,
                          invitations: [
