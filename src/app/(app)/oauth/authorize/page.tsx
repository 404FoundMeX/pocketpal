...
export default async function AuthorizePage({
  ...
  // Mock client details for now - in a real app, you'd use oidcProvider plugin
  const clientDetails = {
    name: "Demo OAuth App",
    icon: null,
  };
            ...
            <CardContent className="p-6">
              <div className="mb-6 flex items-center justify-between rounded-lg bg-zinc-800 p-4">
                <div>
                  ...
                  <div className="text-zinc-400">{session?.user.email ?? "user@example.com"}</div>
                </div>
                <ArrowUpRight className="h-5 w-5 text-zinc-400" />
              </div>
              <div className="flex flex-col gap-1">
                <div className="mb-4 text-lg">
                  Continuing will allow Sign in with {clientDetails.name ?? "this app"} to:
                </div>
